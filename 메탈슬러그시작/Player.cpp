	...
	: GameObject(name, pos, size, pivot)
{
	...
	_playerbullet->Init("플레이어/기본총알.bmp", 100, 40, 30, 1200,true);//프레임이미지 

	
	//===============해비머신건 
	_heavyBullet = new Bullet("플레이어 해비");
	_heavyBullet->Init("플레이어/헤비머신건총알.bmp", 1500, 353, 30, 30000,true);
...
#pragma region _player
	// ========================================== 플레이어 이미지 =======================================================
	IMAGEMANAGER->findImage("플레이어가만");
	IMAGEMANAGER->findImage("플레이어딱총공격");
	IMAGEMANAGER->findImage("플레이어이동");
	IMAGEMANAGER->findImage("플레이어위딱총공격");
	IMAGEMANAGER->findImage("플레이어기본점프");
	IMAGEMANAGER->findImage("플레이어가만위보기");
	IMAGEMANAGER->findImage("플레이어쭈그려");
	IMAGEMANAGER->findImage("플레이어쭈그려서이동");
	IMAGEMANAGER->findImage("플레이어쭈그려공격");
	IMAGEMANAGER->findImage("플레이어좌우이동하며점프");
	IMAGEMANAGER->findImage("플레이어이동하며공격");
	IMAGEMANAGER->findImage("플레이어가만점프공격");
	IMAGEMANAGER->findImage("플레이어이동하면서점프하고위에발사");//발사하는것 
	IMAGEMANAGER->findImage("플레이어점프하면서공격");
	IMAGEMANAGER->findImage("플레이어점프중위공격");
	IMAGEMANAGER->findImage("플레이어이동하며위에공격");
	IMAGEMANAGER->findImage("플레이어가만점프중아래공격");
	IMAGEMANAGER->findImage("플레이어이동점프중아래공격");
    
	//수류탄
	IMAGEMANAGER->findImage("플레이어이동수류탄");
	IMAGEMANAGER->findImage("플레이어가만수류탄던지기");
	IMAGEMANAGER->findImage("플레이어가만점프수류탄");


	//해비머신건   // 160으로 고정한다 
	IMAGEMANAGER->findImage("해비가만");//아직안씀 render에 적긴적음 
	IMAGEMANAGER->findImage("해비이동하며공격");
	IMAGEMANAGER->findImage("해비가만점프공격");
	IMAGEMANAGER->findImage("해비가만공격");
	IMAGEMANAGER->findImage("해비이동점프공격");
	IMAGEMANAGER->findImage("해비가만점프");
	IMAGEMANAGER->findImage("해비아래공격");
	IMAGEMANAGER->findImage("해비아래가만");
	IMAGEMANAGER->findImage("해비아래이동");
	IMAGEMANAGER->findImage("해비이동하며점프");
	IMAGEMANAGER->findImage("해비이동");
	IMAGEMANAGER->findImage("해비이동업샷");
	IMAGEMANAGER->findImage("해비가만업샷");
	IMAGEMANAGER->findImage("해비고정용");

	IMAGEMANAGER->findImage("해비가만아래점프공격");
	IMAGEMANAGER->findImage("해비점프중아래보고가만");
	IMAGEMANAGER->findImage("해비앉아서수류탄");
     
	IMAGEMANAGER->findImage("낙하산");
#pragma endregion
	_lifelive = false; 
			...
			if (_hfire == true)
			{
				if (_time % 3 == 0) //0.03초마다 해비머신건이 발사된다 
				{
					...
					case BULLET::LEFTFIRE://왼쪽만 가는 헤비머신건 
						_heavyBullet->fire(_position.x + 700, RND->range(_position.y + 330, _position.y + 355) /*_position.y*/, _angle, 12.5f);  //20정도의 오차 
						break;
					case BULLET::RIGHTFIRE://오른쪽만 가는 헤비머신건 
						_heavyBullet->fire(_position.x + 935, RND->range(_position.y + 330, _position.y + 355), _angle, 12.5f);
						break;
					case BULLET::UPFIRE://위에보면서 좌르륵가는거 
						...
						case STATE::WALK_UPSHOT:
							if (_isLeft)
							{
								_heavyBullet->fire(RND->range(_position.x + 770, _position.x + 790), _position.y + 270, _angle, 12.5f);
							}
							else if (!_isLeft)
							{

								_heavyBullet->fire(RND->range(_position.x + 880, _position.x + 900), _position.y + 270, _angle, 12.5f);
							}
							break;
						case STATE::WLAK_UPSTARE:
							if (_isLeft)
							{
								_heavyBullet->fire(RND->range(_position.x + 840, _position.x + 860), _position.y + 185, _angle, 12.5f);
							}
							else if (!_isLeft)
							{

								_heavyBullet->fire(RND->range(_position.x + 830, _position.x + 850), _position.y + 185, _angle, 12.5f);
							}
							break;
						}
					...
					case BULLET::DOWNFIRE://점프상태일떄 아래보면서 쏘는거 
						...
						case STATE::JUMP_DOWNSHOT:
							if (_isLeft)
							{
								_heavyBullet->fire(RND->range(_position.x + 800, _position.x + 820), _position.y + 390, _angle1, 12.5f);
							}
							else if (!_isLeft)
							{

								_heavyBullet->fire(RND->range(_position.x + 820, _position.x + 840), _position.y + 390, _angle1, 12.5f);
							}
							break;
					...
					case BULLET::DOWNATTACK://그냥 앉아서 쏘는거 
						...
						if (!_isLeft)
						{
							_heavyBullet->fire(_position.x + 1000, RND->range(_position.y + 330, _position.y + 350), _angle, 12.5f);
						}
						else {
							_heavyBullet->fire(_position.x + 700, RND->range(_position.y + 330, _position.y + 350), _angle, 12.5f);
						}
					case BULLET::IDLE:
						break;
					}
				...
				case WALKSTATE::WALK:
					...
					if (_isLeft == false)
					{
						...
						if (_frameCount % SPEED == 0)
						{
							...
							if (_frameIndex == 0)
							{
								_playerbullet->fire(_position.x+ 183, _position.y+180, 0, 12.5f);
							}
							if (_frameIndex == 5)
							{
								_state = STATE::IDLE;
				...
				case WALKSTATE::IDLE:
					...
					if (_isLeft == false)
					{
						...
						if (_frameCount % SPEED == 0)
						{
							...
							if (_frameIndex == 0)
							{
								_playerbullet->fire(_position.x+ 183, _position.y+ 195, 0, 12.5f);
							}
							if (_frameIndex == 5)
							{
								_state = STATE::IDLE;
			...
			case BULLET::LEFTFIRE:
				...
				case WALKSTATE::JUMPWALK:
					...
					if (_frameCount % SPEED == 0)
					{
						...
						if (_frameIndex == 5)
						{
							_playerbullet->fire(_position.x+15, _position.y+185, PI, 12.5f);
						}
						if (_frameIndex == 0)
						{
							_state = STATE::IDLE;
				...
				case WALKSTATE::WALK:
					...
					if (_isLeft == false)
					{
						...
						if (_frameCount % SPEED == 0)
						{
							...
							if (_frameIndex == 0)
							{
								_playerbullet->fire(_position.x+ 165, _position.y-20, PI / 2, 12.5f);
							}
							if (_frameIndex == 5)
							{
								_state = STATE::IDLE;
				...
				case WALKSTATE::JUMP:
					...
					if (_isLeft == false)
					{
						...
						if (_frameCount % SPEED == 0)
						{
							...
							if (_frameIndex == 0)
							{
								_playerbullet->fire(_position.x+ 155, _position.y+270, PI2 - (PI / 2), 12.5f);
							}
							if (_frameIndex == 5)
							{
								_state = STATE::IDLE;
			...
			case BULLET::DOWNATTACK://플레이어쭈그려공격
				...
				if (_isLeft == false)
				{
					...
					if (_frameCount % SPEED == 0)
					{
						...
						if (_frameIndex == 0)
						{
							_playerbullet->fire(_position.x+280, _position.y + 225, 0, 12.5f);
						}
						if (_frameIndex == 9)
						{
							_state = STATE::CROUCH;
				...
				else//왼쪽모션
				{
					...
					if (_frameCount % SPEED == 0)
					{
						...
						if (_frameIndex == 9)
						{
							_playerbullet->fire(_position.x+200 , _position.y + 225, PI, 12.5f);
						}
						if (_frameIndex == 0)
						{
							_state = STATE::CROUCH;
